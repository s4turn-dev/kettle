...
class Logger:
    ...
    def log_to_txt(self, message: str, prettify: bool = True):
        ...
        :param prettify: если истинно, строка запишется в виде "[ЧАС:МИНУТА] строка" вместо "строка"
        :return: нет
        """
        datetime_mark = dt.today()
        with open(self.txtFilepath, 'a') as file:
            hour_minute_prefix = f"[{datetime_mark.strftime('%H:%M')}] " if prettify else ''
    ...
    def select_last_x_messages_from_db(self, amount: int = 50):
        """
        Возвращает из базы данных последние сообщения в указанном количестве
        ...
        with sqlite3.connect(self.dbFilepath) as db:
            ...
            return (item[0] for item in msgs)
